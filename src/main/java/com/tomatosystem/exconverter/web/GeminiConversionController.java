...
/**
 * Gemini-backed twin of {@link ImageConversionController}. Same multipart contract and the same response
 * JSON, plus {@code engine} and {@code usage}, so an eXBuilder6 Submission only needs a different action URL
 * to switch engines:
 *
 * <pre>
 *   POST /EXConverter/gemini/uploadAndGenerate.do   multipart/form-data, first file part = image
 *   GET  /EXConverter/gemini/status.do              {configured, model, ...} - is the API key set?
 * </pre>
 *
 * The local Ollama endpoint ({@code /EXConverter/uploadAndGenerate.do}) is unchanged.
 */
@Controller
@RequestMapping("/EXConverter/gemini")
public class GeminiConversionController {
	private static final long MAX_IMAGE_BYTES = 20L * 1024L * 1024L;
	private static final Logger LOGGER = LoggerFactory.getLogger(GeminiConversionController.class);
	@Autowired private GeminiUiIrAnalyzer geminiAnalyzer;
	...
	public void status(HttpServletResponse response) throws IOException {
		write(response, GeminiUiIrAnalyzer.describe().toString());
	}
	...
	public void uploadAndGenerate(MultipartHttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			if (!GeminiUiIrAnalyzer.isConfigured()) {
				response.setStatus(HttpServletResponse.SC_SERVICE_UNAVAILABLE);
				write(response, new JSONObject().put("engine", "gemini").put("error", "Gemini API key is not configured (exconverter.gemini.apiKey or GEMINI_API_KEY)").toString());
				return;
			}
			...
			ProgressLog.step("===== [Gemini] 설계서 → CLX 변환 시작: {} ({} KB) =====", originalName, file.getSize() / 1024);
			ImageUiIrAnalyzer.Analysis analysis = geminiAnalyzer.analyze(upload, originalName);
			...
			ProgressLog.step("===== [Gemini] 변환 완료: 총 {}s, 분석모드={} =====", elapsed, analysis.getMode());
			JSONObject body = new JSONObject();
			body.put("id", generated.getId());
			body.put("engine", "gemini");
			...
			body.put("usage", geminiAnalyzer.lastUsage());
			...
			write(response, new JSONObject().put("engine", "gemini").put("error", String.valueOf(e.getMessage())).toString());
		} catch (Exception e) {
			ProgressLog.step("===== [Gemini] 변환 실패: {} =====", e.getMessage());
			LOGGER.error("Gemini image-to-CLX generation failed", e);
