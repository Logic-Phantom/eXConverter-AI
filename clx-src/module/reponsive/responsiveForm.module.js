...
RForm.prototype._transform = function(colCount) {
	...
	var children = that._container.getChildren();		
	...
	switch (that._appInstance.targetScreen.name) {
		case this._screenNms["mobile"]: {
	...
	for (var idx = 0; idx < visibleChildren.length; idx++) {
		 ...
		 */		
//		if (each.id == "btnSearch") { // 조회버튼 식별 로직(each instanceof udc.com.udcComBtnSearch 등)
//			console.log("tt");
//			this._container.replaceConstraint(each, {
//				rowIndex: rowIndex,
//				colIndex: numberOfColumns - 1,
//				colSpan: colSpan,
//				// horizontalAlign : "right", // 우측 정렬 옵션, 일반적으로 width를 지정하지 않는다면 udc 또는 플로우 레이아웃(우측정렬)으로 구성하여 작성하는것을 권장
...
//		}
		...
		if ((that._container.getAppInstance().targetScreen.name == this._screenNms["mobile"] && (that._container.userAttr(that.ATTR_NM.ATTR_MOBILE_COLUMN_COUNT) == 1) || that._container.getAppInstance().targetScreen.name == this._screenNms["tablet"] && that._container.userAttr(that.ATTR_NM.ATTR_TABLET_COLUMN_COUNT) == 1)) {
...
RForm.prototype.setRowVisible = function(psScreenNm, paRowIdx, pbVisible) {
	...
	paRowIdx.forEach(function(rowIdx){
		originalLayout.setRowVisible(rowIdx, pbVisible);
	});
...
RForm.prototype.setColumnVisible = function(psScreenNm, paColIdx, pbVisible) {
	...
	paColIdx.forEach(function(colIdx){
		originalLayout.setColumnVisible(colIdx, pbVisible);
	});
