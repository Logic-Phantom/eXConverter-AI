...
//			}
	...
	/* STEP2 : 지정된 컬럼중 하나 이상 필수 입력 체크(사용자 속성 : xorRequired, 그리드일경우 columnName, 그룹 및 일반컨트롤일 경우 id)*/
	{
	 ...
	 * 하나의 값이 여러개의 컨트롤로 분리되어 있는 경우 사용자속성에 합칠 ID 작성 필요(사용자 속성 : mergeControlIds, 자기 자신은 제외한 컨트롤 id 순서대로 작성 필요)
	 */
	{
	...
	/* 
	 * STEP6 : 두 값을 비교(사용자 속성 : compare, 그리드 일경우 columnName, 일반 컨트롤일 경우 컨트롤 id + 비교연산자)
	 * ex) 시작일시 데이트 인풋 : dtiEnd,<= / 종료일시 데이트 인풋 : dtiStart,>=
	 */
	{
	...
	/* STEP8 : 정수타입별 유효한 값인지 check(사용자 속성 : integer)
	 * small : short integer / normal : integer / big : long integer
	 */
	{
	...
	/* STEP9 : 정수 값 사이에 있는지 check(사용자 속성 : integer-between, 사용자 속성에 직접 최소, 최대값을 설정한다.)*/
	{
	 ...
	 * ex) 입력 예시 : 20230101,20231231 (2023.01.01 ~ 2023.12.31 사이에 있는 날짜인지 확인)
	 */
	{
	 ...
	 * ex) 입력 예시 : 3,2 (정수 최대 3자리, 소수 최대 2자리까지 유효, 0 ~ 999.99)
	 */
	{
